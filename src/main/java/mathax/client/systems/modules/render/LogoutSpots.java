...
public class LogoutSpots extends Module {
    ...
    private final Setting<Boolean> SendCords = sgGeneral.add(new BoolSetting.Builder()
        .name("cords")
        .description("Send cords to chat")
        .defaultValue(false)
        .build()
    );
    ...
    private class Entry {
        ...
        public Entry(PlayerEntity entity) {
            ...
            if (SendCords.get()) {
                leavemessage = "(highlight)%s(default)" + " has left on cords " + "[(highlight)%.1f, %.1f, %.1f(default)]";
                ChatUtils.sendMsg(Formatting.GRAY, leavemessage, name, x, y, z);
            }
        }
